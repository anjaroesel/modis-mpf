...
# run melt_pond_processing_daily.py
# python melt_pond_processing_daily.py
from pylab import *
from scipy import *
...
# 1. download MODIS daily *.hdf files in a specific directory
...
#folder='/scratch/clisap/seaice/MODIS/2012_daily/'
#folder='/scratch/clisap/seaice/MODIS/2009_06_Kara_MOD09GA/'
folder='/scratch/clisap/seaice/MODIS/2009_06_08_North_of_Greenland_MOD09GA/'
#folder='/scratch/clisap/seaice/MODIS/2011_daily/'
#folder='/scratch/clisap/seaice/MODIS/2008_daily/'
#folder='/scratch/clisap/seaice/MODIS/TEST/'
...
#prodfolder='/scratch/clisap/seaice/OWN_PRODUCTS/MELT_PONDS/PRODUCTS/2011_daily/'
#prodfolder='/scratch/clisap/seaice/OWN_PRODUCTS/MELT_PONDS/PRODUCTS/2008_daily/'
#prodfolder='/scratch/clisap/seaice/OWN_PRODUCTS/MELT_PONDS/PRODUCTS/TEST/'
...
def read_MOD_FLAG2(dataset): 
	print('read flags...')
	B={}
	B={'L':array(dataset['0m_2D_QC_500m'].ReadAsArray())}
	band=B.keys
	bands={}
	for i in band():
		bands[i]=B[i]
...
metalist = glob.glob(folder+'A2009179*/')
print metalist
...
for fn in metalist:
	...
	for n in bflag2.flatten():
   	    flag2=binary_repr(n,width=32)
	    if flag2[31]=='0' and flag2[30]=='0':
                q=0 #clear
	    else:
                q=1
	    qc_mask.append(q)
	...
	qc=array(qc_mask).reshape(bflag2.shape)
        maske=clouds+land 
        maske[maske==2]=1
        maske=abs(maske-1)  
...
#####
days=['179']
#days=['240','241','242', '243']
#days=['200','201','202','203','204','205']
#days=['199']
print(days)
#############
#BLENDFILE
...
##METALIST## 
############
metalist_n = glob.glob(workfolder+'20*mosaic/')
print(metalist_n)
...
for dataset in metalist:
    ...
    #check this:!!!
    if not os.path.exists(workfolder+date+'_ow_125.nc' or workfolder+date+'_ow_25.nc' or workfolder+date+'_ow_05.nc'):
	...
	if not os.path.exists(workfolder+date+'_mp_125.nc'):
		...
		os.system('blockmean '+FILENAMEmp+opts(G,['Rx','I'])+' -Eslh -Wo -V > '+outfile)
		os.system('xyz2grd '+outfile+opts(G,['Rx','I'])+' -V -G'+MEANoutfile_mp125)
		...
		cmd('gmtconvert '+outfile+' -F0,1,3 -V > '+SDfile)
		cmd('xyz2grd '+SDfile+opts(G,['Rx','I'])+' -V -G'+SDoutfile_mp125)
		
		cmd('gmtconvert '+outfile+' -F0,1,6 -V > '+WEIGHTfile)
		cmd('xyz2grd '+WEIGHTfile+opts(G,['Rx','I'])+' -V -G'+WEIGHToutfile_mp125)
	...
	if not os.path.exists(workfolder+date+'_ow_125.nc'):
		...
		os.system('blockmean '+FILENAMEow+opts(G,['Rx','I'])+' -Eslh -V > '+outfile)
		os.system('xyz2grd '+outfile+opts(G,['Rx','I'])+' -V -G'+outfile_ow125) 
	...
	if not os.path.exists(workfolder+date+'_sn_125.nc'):
		...
		cmd('gmtset OBLIQUE_ANOTATION 1 PAPER_MEDIA A4+ PAGE_ORIENTATION portrait PLOT_DEGREE_FORMAT ddd:mm:ss -V')
		cmd('gmtset COLOR_NAN 200')
		os.system('blockmean '+FILENAMEsn+opts(G,['Rx','I'])+' -Eslh -V > '+outfile)
		os.system('xyz2grd '+outfile+opts(G,['Rx','I'])+' -V -G'+outfile_ow125) 
